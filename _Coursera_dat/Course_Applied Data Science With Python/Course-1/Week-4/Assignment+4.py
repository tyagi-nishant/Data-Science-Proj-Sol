...
def get_recession_bottom():
    ...
    start=(gdplev[gdplev['Quarter']==get_recession_start()]).index.tolist()[0]
    end=gdplev[gdplev['Quarter']==get_recession_end()].index.tolist()[0]
    bottom=gdplev.iloc[start:end+1]['GDP'].min()
    bot_ind=(gdplev.iloc[start:end+1][gdplev.iloc[start:end+1]['GDP']==bottom]).index.tolist()[0]-start
    return gdplev.iloc[start:end+1].iloc[bot_ind]['Quarter']
...
def convert_housing_data_to_quarters():
    ...
    quat=[list(data.columns)[x:x+3] for x in range(0,len(list(data.columns)),3)]
    coln=new_col_names()
    for col,q in zip(coln,quat):
